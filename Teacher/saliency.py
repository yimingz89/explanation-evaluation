...
def main():
	...
	with torch.no_grad():
		for i, (image, target) in enumerate(val_loader):
			...
			# if display:
			# 	display = transforms.Compose([inv_normalize, transforms.ToPILImage()])((image.squeeze()))
			# 	plt.imshow(display)
			# 	plt.savefig(save_path + 'original.pdf')
			...
			if target_label == teacher_label and target_label not in seen_targets:
				save_path = './examples-90/' + str(target_label) + '.pdf'
				freq_path = './examples-90/freq' + str(target_label) + '.pdf'
				...
				explanations = [('original', image), ('middle layer', middle_explanation), ('edge detector', edges), ('gradient', grad), ('smoothgrad', smooth_grad), ('gradcam', gradcam), ('lime', lime), ('shap', shap)]
